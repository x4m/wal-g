...
func generateData(t *testing.T) string {
	...
	f, err := os.Create(filepath.Join(dir, "global", internal.PgControl))
	if err != nil {
		t.Log(err)
	}
	err = f.Chmod(0600)
...
func extract(t *testing.T, dir string) string {
	...
	ft := internal.NewFileTarInterpreter(outDir, internal.BackupSentinelDto{}, map[string]bool{
		"/1":                 true,
		"/2":                 true,
		"/3":                 true,
		"/4":                 true,
...
func compare(t *testing.T, dir1, dir2 string) bool {
	...
	for i, f2 := range files2 {
		...
		if f2.IsDir() {
			_, ok := internal.ExcludedFilenames[f2.Name()]
			if ok {
