...
func TestGetBinlogConfig(t *testing.T) {
	...
	time, path := internal.GetBinlogConfigs()
...
func TestBinlogShouldBeFetched(t *testing.T) {
	...
	dto := internal.StreamSentinelDto{BinLogStart: "mysql-bin-log.000018", BinLogEnd: "mysql-bin-log.000019.lz4"}

	assert.NoError(t, err)
	assert.Equal(t, len(objects), 4)
	for _, o := range objects {
		binlogName := internal.ExtractBinlogName(o, folder)
		fetched := internal.BinlogShouldBeFetched(dto, binlogName, &cutpoint, o)
		if fetched {
