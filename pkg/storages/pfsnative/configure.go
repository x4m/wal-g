...
func ConfigureStorage(ctx context.Context, prefix string, settings map[string]string, rootWraps ...storage.WrapRootFolder) (storage.HashableStorage, error) {
	...
	return newStorage(ctx, config{RootPath: rootPath, PBDName: pbdName, Cluster: cluster, HostID: hostID, Server: settings[serverSetting], Timeout: timeout}, rootWraps...)
}
